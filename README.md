...
When you're running a multi-account AWS architecture, the question isn't if you need centralized observability 
it's how you'll achieve it without drowning in manual configuration.
...
![central-monitoring](docs/central-monitoring.png)
...
![aws-architecture.jpg](docs/aws-architecture.jpg)
...
Since our Terraform code uses a data source to read the Organization (rather than importing and managing the resource directly), Terraform cannot enable Policy features for us.
We must enable them manually to attach our security guardrails.
...
![AWS Organization](./docs/aws-organization.png)
...
Delegated administration empowers the Observability account to manage CloudFormation StackSets across the organization,
but we must ensure this power is limited to the correct OU.
...
This SCP denies any CloudFormation actions performed by the service-managed StackSets execution role unless the target account is inside the Workloads OU.
This ensures that delegated automation cannot accidentally modify shared services, identity, or security infrastructure.
...
Next, we will set up CloudWatch Observability Access Manager (OAM) in the central observability account and automatically
deploy OAM links to all workload accounts using CloudFormation StackSets.
...
With Organization infrastructure initialized and our Cross-Account Observability automation listening for new accounts, 
it is time to deploy the actual "Coffee Shop" Microservices.
...
![observability-account-dev-log-groups](docs/observability-account-dev-log-groups.png)
...
![observability-account-staging-log-groups](docs/observability-account-staging-log-groups.png)
